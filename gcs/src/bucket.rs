...
async fn remote_snapshot_metas(bucket: &str) -> anyhow::Result<Vec<ObjectResponse>> {
    info!("Fetching snapshots from GCS, this usually takes 60-90s");
    let pre = Instant::now();
    let resp = get_all_objects(bucket, None).await?;
    info!("Fetched all GCS objects in {}s", pre.elapsed().as_secs());
    // cache response on disk to speed up load times in the future
...
async fn local_snapshot_metas(path: &str) -> anyhow::Result<Vec<ObjectResponse>> {
...
pub async fn get_snapshot_metas(src: GcsObjectsSource) -> anyhow::Result<Vec<SnapshotMeta>> {
    let resp = match src {
        GcsObjectsSource::Url(url) => remote_snapshot_metas(&url).await,
        GcsObjectsSource::Path(path) => local_snapshot_metas(&path).await,
    }?;
